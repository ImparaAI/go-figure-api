...
func TestMissingImage(t *testing.T) {
	...
	assert.True(t, response.IsBadRequest())
	assert.Equal(t, `{"message":"The image is invalid."}`, response.Body())
}
...
func TestBadImage(t *testing.T) {
	json := `{"points":[{"x": 0, "y": 0, "time": 0}], "image": "bad image"}`
	response := requester.Post("/drawing", json)
	assert.True(t, response.IsBadRequest())
	assert.Equal(t, `{"message":"The image is invalid."}`, response.Body())
}
...
func TestEmptyPointsField(t *testing.T) {
	response := requester.Post("/drawing", `{"points":[], "image": "data:image/png;"}`)
	assert.True(t, response.IsBadRequest())
...
func TestInvalidPoints(t *testing.T) {
	...
	for _, val := range nonInts {
		response := requester.Post("/drawing", `{"points":[` + val + `], "image": "data:image/png;"}`)
		assert.True(t, response.IsBadRequest())
		assert.Equal(t, `{"message":"The request is not properly formatted."}`, response.Body())
	}
}
...
func TestFirstPointTimeNonZero(t *testing.T) {
	json := `{"points":[{"x": 4, "y": 5, "time": 0.5}, {"x": 5, "y": 1, "time": 1}], "image": "data:image/png;"}`
	response := requester.Post("/drawing", json)
	assert.True(t, response.IsBadRequest())
...
func TestNonSequentialTimes(t *testing.T) {
	json := `{"points":[{"x": 4, "y": 5, "time": 0}, {"x": 5, "y": 1, "time": 0}, {"x": 2, "y": 3, "time": 1.5}, {"x": 6, "y": 3, "time": 1.1}], "image": "data:image/png;"}`
	response := requester.Post("/drawing", json)
	assert.True(t, response.IsBadRequest())
...
func TestSubmitSuccess(t *testing.T) {
	...
	json := `{"points":[{"x": 4, "y": 5, "time": 0}, {"x": 5, "y": 1, "time": 0}, {"x": 2, "y": 3, "time": 1.5}, {"x": 6, "y": 3, "time": 2.1}], "image": "data:image/png;"}`
	response := requester.Post("/drawing", json)
